...
- `Public/Private` ：这里需要填写仓库的模式 ，`Public` 表示公开，意味着任何人都可以查看该仓库中的所有代码，`Private` 表示私有，意味着只有仓库拥有者有权查看该仓库中的代码。在 GitHub 中允许我们创建一定数量的私有仓库。
- `Initialize this repository with the README`: 表示是否在创建仓库时创建 README 文件，如果勾选，表示创建，反之不创建。

在创建完成仓库后，我们会得到如下界面：

![GitHub_创建仓库3.png](https://upload-images.jianshu.io/upload_images/2824145-f1aa22c6d4a7fe8b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

在GitHub中有三种方式来处理远程仓库与本地仓库的关联：

- 第一种，在本地创建新的仓库并与远程仓库关联。
- 第二种，将已有的本地仓库中的内容推送到现在的GitHub新建的远程仓库中。
- 第三种，从已有的仓库中导入代码，使用这种模式需要我们提供相应的URL。

大家可以根据实际的需求来选择不同的方式来关联远程仓库，当然在该界面中已经提供了关联远程仓库的相关指令。

#### 本地仓库如何与远程仓库关联

在上述文章中，我们只介绍了如何创建远程仓库，并没有介绍如何将本地仓库与远程仓库进行关联。在节中我已经介绍了关联远程仓库的方式，那下面我们将使用第二种方式，也就是将已有的本地仓库与远程仓库进行关联。

因为我已经创建了本地仓库 `GitTestProject` ，那么根据之前 GitHub 中给我们的提示，我们需要使用命令：

```bash
git remote add origin git@github.com:AndyJennifer/GitTestProject.git
```

使用上述命令，就可以将本地仓库与远程仓库建立连接了。对于该命令，我们几点需要注意：

- 该命令格式为 `git remote add 远程仓库简写名 远程仓库地址`
- `origin` 这个单词只是指代远程仓库，你可以当做它是远程仓库的别名，一般情况下都是使用 `origin` 来指代远程仓库，当然你可以将它修改为其他名称，比如 `hahah` ，那么我们需要修改命令为：

 ```bash
 git remote add haha git@github.com:AndyJennifer/GitTestProject.git
 ```
...
![git_remote.png](https://upload-images.jianshu.io/upload_images/2824145-179f97922c204e4d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
