...
克隆该项目后，我们通过cd命令进入该项目,使用`git log`命令，我们能得到下列输出：
...
如果你是第一次使用`git log`命令，可能会有一个疑问，如何加载更多的历史记录呢？如何退出呢？如果大家仔细观察，在我们的界面中末尾有个冒号`：`，该冒号表明还可以显示更多的输出行。在Git中是使用`Less`程序作为其分页器，如果你不熟悉less或分页器也没有问题。大家只要知道该分页器是用于翻页并浏览内容的就行了。下面我们看看该分页器对应的按键指令。
...
在上文中，我么已经通过添加 `--stat` 选项，可以知道修改了哪些文件以及添加/删除了多少行代码。如果能查看文件中到底进行了哪些更改，是不是更好呢？在`git log`命令中具有一个可用来显示对文件作出实际更改的选项，就是`--patch`，可以简写为`-p`。
...
- 第二个：显示了第一个版本哈希值与更改后的哈希值，这些哈希值和提交的`SHA`是不同的。
...
- 第五个：表示了在commit中实际进行的更改。
  - 用红色并以减号(`-`)开头的行是位于文件原始版本中，但是被 commit 删除的行
...
这里我们可以提供完整的`SHA`，或`SHA`的`前七个`字符，作为具体的查询条件。需要注意的是使用`git log`+`SHA`的这种方式，并不会单独的某个提交，而是命令行将从这条提交开始输出历史记录，你仍然可以通过Git提供的分页器，查看该条提交信息之后的记录。
