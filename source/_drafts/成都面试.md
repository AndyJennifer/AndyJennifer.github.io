...
### 快手一面
- 问题：StringBuffer与StringBuilder
- 问题：HashMap时间复杂度。
- 问题：事件分发机制
- 问题：Handler机制，细分到源码
- 问题：Handler内存泄露的原因及其解决方法。
- 问题：硬件加速
- 问题：AsyncTask异步任务
- 问题：JVM相关
- 问题：事件冲突的解决
- 问题：算法题
- 问题：Java中的几种引用关系，强，软，弱，虚
...
- 问题：注解的原理
- 问题：对kotlin语言的理解
- 问题：kotlin与Java 8之间的区别

### 1919
- 问题1：给匿名内部类传递参数的时候，使用的形参为何要用final修饰

- 问题2：内部类，静态内部类，匿名内部类的关系及含义

#### 问题1
因为是引用传递，为了保证传递的参数的正确性，所以必须使用final修饰

#### 问题2
内部类：定义一个类内部的类叫做内部类，包含内部类的类叫做外部类。外部类可以访问内部类的所有方法与属性。包含私有方法和属性。
静态内部类：只是内部类添加了static修饰。且静态内部类不能访问外部类的非静态成员