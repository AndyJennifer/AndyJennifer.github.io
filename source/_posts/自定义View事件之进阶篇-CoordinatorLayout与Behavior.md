...
- A控件的高度为`match_parent`或者`wrap_content`。
- A、B控件的嵌套滑动关系为：B控件先处理嵌套滑动事件，当控件B向上滑动至隐藏后，控件A才能开始滑动。

那么根据上述条件，在滚动的过程中，我们会发现一个问题，就是当我们的控件A逐渐滑动到顶部时，我们会发现屏幕下方会出现一个**空白区域**，那原因是什么呢？其实很简单，当控件A高度为``match_parent`或者`wrap_content``时，根据View的测量规则，控件A实际的高度就是整个控件剩余的高度（屏幕高度-控件B的高度），所以当控件B滚出屏幕后，那么就会出现一段空白。
